...
router.post('/panier', (req, res) => {
  ...
  if (isNaN(quantity) || quantity <= 0) {
    res.status(400).json({ message: "Please add a valid quantity"})
    return
  }
  ...
  if(!article) {
    res.status(404).json({ message: "The is of this article does not exist"})
    return
  }
  ...
  if (articleInPanier) {
    res.status(400).json({ message: "The article already exists in your panier"})
    return
  }
...
router.post('/panier/pay', (req, res) => {
  ...
  req.session.destroy()
  res.send({ message: `Merci ${firstname} ${lastname} pour votre achat`})
})
...
router.delete('/panier/:articleId', (req, res) => {
  ...
  if (articleId <=0){
    res.status(400).json({ message: "Please add a valid id"})
    return
  }
